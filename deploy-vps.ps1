# ===========================================
# NOTARYPRO BACKEND - DESPLIEGUE EN VPS (POWERSHELL)
# ===========================================

Write-Host "🚀 Iniciando despliegue en VPS..." -ForegroundColor Green

# 1. Conectar al VPS y ejecutar el script de despliegue
...
Write-Host "📝 Script de despliegue creado. Ahora ejecuta manualmente:" -ForegroundColor Cyan
Write-Host "1. Conecta al VPS: ssh root@170.239.86.119 -p22222" -ForegroundColor White
Write-Host "2. Copia el archivo env.vps al servidor" -ForegroundColor White
Write-Host "3. Ejecuta: bash deploy-temp.sh" -ForegroundColor White

Write-Host "`n📋 Comandos para ejecutar en el VPS:" -ForegroundColor Yellow
Write-Host "cd /var/www/notarypro-backend" -ForegroundColor Gray
Write-Host "cp env.vps .env" -ForegroundColor Gray
Write-Host "bash deploy-temp.sh" -ForegroundColor Gray

Write-Host "`n🔍 Para verificar el estado después del despliegue:" -ForegroundColor Yellow
...
Write-Host "`n✅ Script de despliegue preparado!" -ForegroundColor Green
